...
export default function VendorTable({ refreshKey, onRefresh }: Props) {
  ...
  const fetchVendors = useCallback(async () => {
    if (!hasMore && filters.page > 1) return;
    ...
    try {
      ...
      if (filters.name) {
        data = await useSearchVendor({ name: filters.name });
      } else {
    ...
    <div className="bg-white shadow-sm rounded-2xl p-6 border border-gray-200">
      ...
      <VendorDetailModal
        ...
        onUpdate={(updated) => {
          ...
          onRefresh(); // ← обновляем после редактирования
        }}
      />
    </div>
  );
