...
// components/CreateVendorModal.tsx
'use client';
...
interface Props {
  onClose: () => void;
  onCreate: () => void; // ← теперь просто колбэк
}
...
export default function CreateVendorModal({ onClose, onCreate }: Props) {
  ...
  const handleChange = (field: keyof VendorCreateInput, value: string | number) => {
    setForm((prev) => ({ ...prev, [field]: value }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (created) {
      onCreate(); // ← вызываем родительский колбэк
      onClose();
    } else {
      setError('Ошибка при создании поставщика.');
    }
  };
        ...
        >
          ...
          <div className="flex justify-end mt-6 gap-3">
            ...
            <button
              ...
              disabled={creating}
              className={`px-4 py-2 rounded-lg flex items-center gap-2 text-white transition ${
                creating ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700'
              }`}
            >
              <Plus className="w-4 h-4" />
              {creating ? 'Создаём...' : 'Создать'}
